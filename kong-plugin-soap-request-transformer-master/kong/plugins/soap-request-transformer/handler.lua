...
local function remove_unwanted_elements(e, unwanted_attrs, unwanted_elements)
    if type(e) == "table" then
        for k, v in pairs(e) do
            -- Supprimer les attributs non désirés
            if unwanted_attrs[k] then
                e[k] = nil
            else
                -- Nettoyer les sous-éléments
                remove_unwanted_elements(v, unwanted_attrs, unwanted_elements)
            end
        end
    end
end
...
function SoapTransformerHandler.convertXMLtoJSON(xml, conf)
    ...
    -- Définir les attributs et éléments non désirés à supprimer
    local unwanted_attrs = {
        ["xsi:nil"] = true,
        ["xsi:type"] = true
    }
   
    local unwanted_elements = {
        -- Liste des éléments à supprimer si nécessaire
    }
   
    -- Nettoyer les éléments non désirés
    remove_unwanted_elements(t, unwanted_attrs, unwanted_elements)
