...
from lxml import etree  # Add this import statement
import logging
...
def soap_api():
    ...
    print(f"Received SOAP request: {soap_request.decode()}")
    logging.debug("Received SOAP request:")
    logging.debug(soap_request.decode())  # Utilisez logging au lieu de print
    # Validate if the request contains the expected structure and content
    expected_identification = "66559f7f9846ec79fe26fdea|APIMAltares2024!"
    expected_refClient = "Test"
    expected_sirenSiret = "572014199"
    ...
    try:
        ...
        namespaces = {
            ...
            'ser': 'http://serviceobject.service.callisto.newsys.altares.fr',
            'xsd': 'http://request.callisto.newsys.altares.fr/xsd'
        }
        identification = root.find('.//xsd:identification', namespaces).text
        refClient = root.find('.//xsd:refClient', namespaces).text
        sirenSiret = root.find('.//xsd:sirenSiret', namespaces).text

        if (identification == expected_identification and
            refClient == expected_refClient and
            sirenSiret == expected_sirenSiret):
            return Response(soap_response, mimetype='text/xml')
        else:
            return Response("Invalid request content", status=400)
